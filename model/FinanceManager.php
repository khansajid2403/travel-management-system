...
class FinanceManager {
	...
	function loginCheck($email, $password) {
		$sql = "SELECT * FROM finance_manager WHERE email = ?;";
	...
	function registerUser($name, $email, $phone, $password, $profile_pic = null, $profile_pic_type = null) {
		$sql = "INSERT INTO finance_manager (name, email, phone, password, profile_pic, profile_pic_type) VALUES (?, ?, ?, ?, ?, ?);";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
	...
	function getById($id) {
		$sql = "SELECT * FROM finance_manager WHERE finance_id = ?;";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
	...
	function updateProfile($id, $name, $phone) {
		$sql = "UPDATE finance_manager SET name = ?, phone = ? WHERE finance_id = ?;";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
	...
	function updateProfilePic($id, $profile_pic, $profile_pic_type) {
		$sql = "UPDATE finance_manager SET profile_pic = ?, profile_pic_type = ? WHERE finance_id = ?;";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
	...
	function updatePassword($id, $hashed_password) {
		$sql = "UPDATE finance_manager SET password = ? WHERE finance_id = ?;";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
	...
	function deleteById($id) {
		$sql = "DELETE FROM finance_manager WHERE finance_id = ?;";
		$connection = $this->establishConnection();
		$prepared_statement = $connection->prepare($sql);
		$prepared_statement->bind_param('i', $id);
		return $prepared_statement->execute();
