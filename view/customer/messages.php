...
$agents = $log->getAgentsForCustomer($_SESSION['user_id']);

$selected_agent_id = isset($_GET['with']) ? $_GET['with'] : null;
$conversation = [];
if ($selected_agent_id) {
	$conversation = $log->getConversation($selected_agent_id, $_SESSION['user_id']);
}
?>
<!DOCTYPE html>
<html lang='en'>
...
<body>
	...
	<label for="agent_select">Choose a Travel Agent</label>
	<select id="agent_select" onchange="window.location.href='./messages.php?with=' + this.value;">
		<option value="">-- Select Agent --</option>
		<?php foreach ($agents as $a): ?>
		<option value="<?php echo $a['agent_id']; ?>" <?php echo ($selected_agent_id == $a['agent_id']) ? 'selected' : ''; ?>>
	...
	<?php if ($selected_agent_id): ?>
		<div class="card" style="max-width: 600px;">
			<?php foreach ($conversation as $msg): ?>
				<p><strong><?php echo $msg['sender_role'] == 'customer' ? 'You' : 'Agent'; ?>:</strong> <?php echo htmlspecialchars($msg['message']); ?></p>
			<?php endforeach; ?>
